...
const Prepare = (props: Props) => {
  const {navigation, route, isValid, canRehearse,...rest} = props;
  return (
    <View style={{flex: 1}}>
      <Tab.Navigator
        tabBar={(props) => {
          const {descriptors, state, navigation} = props;
          return (<View style={{...biggerShadowStyle,backgroundColor: '#ffffff',flexDirection: 'row', overflow: 'visible'}}>
            {state.routes.map((route, index) => {
              ...
              const Icon = options && options.tabBarIcon && options.tabBarIcon({color: 'black', focused: true});
              const disabled = (index === 1 && !isValid) || (index === 2 && !canRehearse);
              return (
                <TouchableOpacity
                  key={index}
                  disabled={disabled}
                  ...
                  style={{opacity: disabled?.25:1,flex: 1, paddingVertical: 15, justifyContent: 'center', alignItems: 'center'}}
                >
                  {Icon}
                  <Text style={{top: 5,
                  ...
                  {isFocused && <View style={{height: 2, position: 'absolute', bottom: 0, backgroundColor: colors.activeIcon, width: '100%'}} />}
                </TouchableOpacity>
              );
            })}
          </View>)
        ...
        screenOptions={({route}) => ({
          tabBarIcon: () => {
            return route.name === topTabs.TALKING_POINTS ? <MaterialIcons style={{color: colors.activeIcon}} name="record-voice-over" size={20} /> :
              <MaterialCommunityIcons style={{color: colors.activeIcon}} name={route.name === topTabs.DETAILS ? 'file-document-box-outline' : 'microphone'} size={20} />;
          },
        })}
      >
        <Tab.Screen
